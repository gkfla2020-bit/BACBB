...
기존 BACB 전략은 Total Beta만을 고려하여 **일시적 할인율 변동(Discount Rate News)**과 **영구적 현금흐름 충격(Cash-Flow News)**을 구분하지 못하는 한계가 있다.

본 연구는 **VAR 모델**과 **Campbell-Shiller 분해**를 통해 시장 수익률을 분해하고, 각 자산의 **Cash-Flow Beta(β<sub>CF</sub>)**를 추정하여 "진정한 나쁜 베타"를 식별한다.
...
`Cryptocurrency` `Factor Investing` `Bad Beta` `Cash-Flow Beta` `VAR Model` `Campbell-Shiller` `Frazzini-Pedersen` `Low Beta Anomaly`
