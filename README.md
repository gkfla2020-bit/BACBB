...
본 연구는 **Frazzini & Pedersen(2014)**의 저베타(Betting Against Beta, BAB) 전략과 **Campbell & Vuolteenaho(2004)**의 '나쁜 베타(Bad Beta)' 개념을 결합하여 암호화폐 시장에 특화된 **BACBB(Betting Against Cryptocurrency Bad Beta)** 투자 전략을 제안한다.

기존 BACB 전략은 총 베타(Total Beta)를 기준으로 저베타 자산을 매수하고 고베타 자산을 매도하는 포트폴리오를 구성하나, 이는 일시적인 할인율 변동(Discount Rate News)과 영구적인 가치 훼손인 현금흐름 충격(Cash-Flow News)을 구분하지 못한다는 한계가 있다. 본 연구는 이러한 한계를 극복하기 위해 **VAR(Vector Autoregression) 모델**과 **Campbell-Shiller 분해** 기법을 적용하여 시장 수익률을 $N_{CF}$와 $N_{DR}$로 분해하고, 각 자산의 Cash-Flow Beta($\beta^{CF}$)를 추정함으로써 **"진정한 나쁜 베타"**를 식별한다.

2021년 1월부터 2026년 1월까지 주요 암호화폐 50종을 대상으로 실증 분석을 수행한 결과, BACBB 전략은 **연평균 수익률 14.14%**, **샤프 비율 1.04**를 달성하였다. 특히 최대 낙폭(MDD)은 **-16.15%**로 기존 BACB 전략(-44.12%) 대비 리스크 관리 능력이 약 **3배 향상**되었음을 입증하였다. 분석 결과는 $t = 2.79$ $(p = 0.0054)$로 **유의수준 1%에서 통계적 유의성**을 확보하였으며, **표본 외(Out-of-Sample) 검증**에서도 샤프 비율 1.09 $(p = 0.037)$를 기록하여 전략의 견고성과 과적합 부재를 확인하였다.

본 연구는 암호화폐 시장에 "나쁜 베타" 개념을 **최초로 적용**하고 VAR 모델 기반의 $\beta^{CF}$ 추정 방법론을 제시했다는 학술적 의의를 지닌다. 또한 실제 **거래비용(0.04%) 및 펀딩비**를 반영한 시뮬레이션을 통해 실무적 전략 운용 가능성을 입증하였다.
...
| # | Contribution |
|:---:|:---|
| **1** | 암호화폐 시장에 **Bad Beta ($\beta^{CF}$) 개념을 최초 적용** |
| **2** | **VAR 모델 기반 Cash-Flow Beta 추정 방법론** 제시 |
| **3** | 실제 **거래비용(0.04%) 및 펀딩비**를 반영한 실무적 전략 구현 |
| **4** | **Out-of-Sample 검증**으로 과적합 부재 및 전략 견고성 입증 |
| **5** | **통계적으로 유의한 초과수익** 달성 ($t=2.79$, $p<0.01$) |

### 🔑 Keywords


`Cryptocurrency` `Factor Investing` `Bad Beta` `Cash-Flow Beta` `VAR Model` `Campbell-Shiller` `Frazzini-Pedersen` `Low Beta Anomaly` `Out-of-Sample Validation`
